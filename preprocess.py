...
def preprocess(data_path="data.csv", lag=10, lead_time=LEAD_TIME):
    # Read the data
    ...
    adjusted_data = pd.DataFrame({'DATE': np.repeat(date_range, len(all_skus)), 'SKU': np.tile(all_skus, len(date_range))})
    ...
    adjusted_data['target'] = adjusted_data.groupby('SKU')['QUANTITY_SOLD'].transform(lambda x: x.shift(-lead_time).rolling(window=lead_time).sum())

    # Feature engineering
    def add_features(data, lag, lead_time):
        ...
        data['moving_avg_last_2_month'] = data.groupby('SKU')['QUANTITY_SOLD'].transform(lambda x: x.rolling(window=60).mean())
        data['moving_avg_last_month'] = data.groupby('SKU')['QUANTITY_SOLD'].transform(lambda x: x.rolling(window=30).mean())
        data['moving_avg_last_3_days'] = data.groupby('SKU')['QUANTITY_SOLD'].transform(lambda x: x.rolling(window=3).mean())
        data['moving_avg_last_5_days'] = data.groupby('SKU')['QUANTITY_SOLD'].transform(lambda x: x.rolling(window=5).mean())
        data['moving_avg_last_10_days'] = data.groupby('SKU')['QUANTITY_SOLD'].transform(lambda x: x.rolling(window=10).mean())
        data['moving_avg_last_year'] = data.groupby('SKU')['QUANTITY_SOLD'].transform(lambda x: x.rolling(window=365).mean())
        ...
        data['global_moving_avg_last_month'] = data.groupby('DATE')['global_sum_qty'].transform(lambda x: x.rolling(window=30).mean())
        data['global_lag_last_month'] = data.groupby('DATE')['global_sum_qty'].shift(30)
        data['global_moving_avg_3_days'] = data.groupby('DATE')['global_sum_qty'].transform(lambda x: x.rolling(window=3).mean())
        ...
        data['global_moving_avg_last_week'] = data.groupby('DATE')['global_sum_qty'].transform(lambda x: x.rolling(window=7).mean())

        return data.dropna()

    data_ml = add_features(adjusted_data, lag, lead_time)
    ...
    train_data.sort_values(by=['SKU', 'DATE']).to_csv("train_data.csv", index=False)
    test_data.sort_values(by=['SKU', 'DATE']).to_csv("test_data.csv", index=False)
